...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def enable_low_vram_mode(self):
        ...
        This mode:
        - Enables gradient checkpointing in backbone (trades computation for memory)
        - Clears CUDA cache between operations
        - Uses more aggressive memory management

        Note: May slightly reduce inference speed but significantly reduces VRAM.
        """
        self.low_vram_mode = True

        # Enable gradient checkpointing in the backbone
        if hasattr(self.model, 'backbone') and \
                hasattr(self.model.backbone, 'pretrained'):
            if hasattr(self.model.backbone.pretrained,
                       'enable_gradient_checkpointing'):
                self.model.backbone.pretrained.enable_gradient_checkpointing()

        # For nested models
        if hasattr(self.model, 'da3'):
            if hasattr(self.model.da3, 'backbone') and \
                    hasattr(self.model.da3.backbone, 'pretrained'):
                if hasattr(self.model.da3.backbone.pretrained,
                           'enable_gradient_checkpointing'):
                    self.model.da3.backbone.pretrained.\
                        enable_gradient_checkpointing()

        if hasattr(self.model, 'da3_metric'):
            if hasattr(self.model.da3_metric, 'backbone') and \
                    hasattr(self.model.da3_metric.backbone, 'pretrained'):
                if hasattr(self.model.da3_metric.backbone.pretrained,
                           'enable_gradient_checkpointing'):
                    self.model.da3_metric.backbone.pretrained.\
                        enable_gradient_checkpointing()
    ...
    def _run_model_forward(
        ...
        feat_layers = list(export_feat_layers) \
            if export_feat_layers is not None else None
